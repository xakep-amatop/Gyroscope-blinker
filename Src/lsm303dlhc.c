...
/** @defgroup LSM303DLHC_Private_Variables
  * @{
  */
MAGNETO_DrvTypeDef Lsm303dlhcDrv_M =
      {
        LSM303DLHC_MagInit,
...
ACCELERO_DrvTypeDef Lsm303dlhcDrv_A =
      {
        LSM303DLHC_AccInit,
...
/**
  ...
  * @retval None
  */
void LSM303DLHC_MagInit(uint32_t InitStruct){
  ...
  cra_regm  = (uint8_t) (InitStruct & 0xff);

  /* Configure MEMS: full Scale */
  crb_regm  = (uint8_t) ((InitStruct >>  8) & 0xff);

  /* Configure MEMS: working mode */
  mr_regm   = (uint8_t) ((InitStruct >> 16) & 0xff);
...
/**
  ...
  * @retval None
  */
void LSM303DLHC_MagReadXYZ(volatile float * pData){
 ...
 buffer[5] = COMPASSACCELERO_IO_Read(MAG_I2C_ADDRESS, LSM303DLHC_OUT_Z_L_M);
  ...
  for(i=0; i<2; i++){
    pData[i]=(float)((int16_t)(((uint16_t)buffer[2*i] << 8) + buffer[2*i+1])*1000)/Magn_Sensitivity_XY;
  }

  pData[2]=(float)((int16_t)(((uint16_t)buffer[4] << 8) + buffer[5])*1000)/Magn_Sensitivity_Z;
}
...
uint8_t LSM303DLHC_AccReadID(void){
  ...
  HAL_I2C_Mem_Read(&I2cHandle, ACC_I2C_ADDRESS, LSM303DLHC_WHO_AM_I_ADDR, I2C_MEMADD_SIZE_8BIT, &ctrl, 1, I2cxTimeout);
...
/**
  ...
  * @retval None
  */
void LSM303DLHC_AccReadXYZ(volatile float * pData){
